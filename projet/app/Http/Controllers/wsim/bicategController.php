...
class bicategController extends Controller
{
 ...
 public function index(Request $request)
 {
  ...
  return view ('wsim.BI.acesso_negado');
 ...
 if($request)
 {
   $categorias = DB::table('bi_categ')->where('nome', 'LIKE', '%' . $request->criterio . '%')
                                      ->orderBy('id', 'asc')
                                     ->paginate(7);
                                      }
 ...
 {
   $categorias = bicateg::paginate(7);


 } 

 return view ('wsim.bicateg.index',compact('categorias','form'));
}
...
public function create()
{
 ...
 return view ('wsim.bicateg.create');
}
